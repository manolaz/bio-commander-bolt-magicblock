...
pub mod play {
    ...
    pub fn execute(ctx: Context<Components>, args: Args) -> Result<Components> {
        ...
        require!(players.players[0] == Some(authority) || players.players[1] == Some(authority), BioCommanderError::NotInGame);
        require!(grid.state == grid::GameState::Active, BioCommanderError::NotActive);
        let player_idx : usize = if players.players[0] == Some(authority) { 0 } else { 1 };
        require!(grid.is_first_player_turn == (player_idx == 0), BioCommanderError::NotPlayersTurn);
        ...
        match args {
            tile @ Args {
                ...
                Some(_) => return Err(BioCommanderError::TileAlreadySet.into()),
                None => {
                    grid.board[tile.row as usize][tile.column as usize] =
                        Some(grid::Sign::from_usize(player_idx));
                }
            },
            _ => return Err(BioCommanderError::TileOutOfBounds.into()),
        }
